"""IB MCP server: paper/real bearer dispatch over a shared endpoint.

The bearer token presented at request time selects which IBService
(paper or real) handles the call. The two modes share the same MCP
...
from quant_arena.config import IBConfig
from quant_arena.ib_service import (
    IBAccountValueInfo,
    IBFillInfo,
    IBOrderType,
    IBPositionInfo,
...
def wrap_ib_mcp_with_token_auth(
    mcp_app: ASGIApp,
    config: IBConfig,
    get_paper: Callable[[], IBService],
    get_real: Callable[[], IBService],
) -> ASGIApp:
    """Bearer-token wrapper that dispatches paper/real and enforces single-client-per-mode."""
    ...
    async def authenticated_app(scope: Scope, receive: Receive, send: Send) -> None:
        ...
        if mode is None or service is None or lock is None:
            response = JSONResponse(
                status_code=401,
                content={"detail": "Invalid IB token"},
            )
