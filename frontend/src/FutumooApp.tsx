...
export function FutumooApp() {
  ...
  async function refreshAgents(preferredAgentId?: string) {
    ...
    setError("");
    try {
      const data = await apiFetch<AgentResponse[]>("/api/futumoo/agents");
      setAgents(data);
      startTransition(() => {
        const nextAgentId =
  ...
  async function refreshSnapshot(agentId: string) {
    ...
    setError("");
    try {
      const data = await apiFetch<AgentSnapshotResponse>(`/api/futumoo/agents/${agentId}`);
      setSnapshot(data);
    } catch (fetchError) {
      setError((fetchError as Error).message);
      setSnapshot(null);
  ...
  async function refreshReports(agentId: string) {
    ...
    try {
      const data = await apiFetch<DailyReportPage>(
        `/api/futumoo/agents/${agentId}/daily-reports?page=1&page_size=${REPORTS_PAGE_SIZE}`,
      );
      setReportsList(data);
    } catch (fetchError) {
      setError((fetchError as Error).message);
  ...
  async function loadReportDetail(agentId: string, tradeDate: string) {
    ...
    try {
      const data = await apiFetch<DailyReport>(
        `/api/futumoo/agents/${agentId}/daily-reports/${tradeDate}`,
      );
  ...
  async function refreshRankings() {
    ...
    try {
      const data = await apiFetch<RankingEntry[]>("/api/futumoo/rankings");
      setRankings(data);
    } catch (fetchError) {
      setError((fetchError as Error).message);
    } finally {
  ...
  async function handleCreateAgent(event: React.FormEvent<HTMLFormElement>) {
    ...
    setCreatedAgentId("");
    try {
      const created = await apiFetch<AgentCreatedResponse>("/api/futumoo/agents", {
        method: "POST",
        body: JSON.stringify({
          ...createAgentForm,
  ...
  async function handleDeleteAgent(agentId: string) {
    const confirmed = window.confirm(`Delete futumoo agent ${agentId}?`);
    if (!confirmed) {
      return;
    }
    setMessage("");
    ...
    setCreatedAgentId("");
    try {
      await apiFetch<void>(`/api/futumoo/agents/${agentId}`, { method: "DELETE" });
    ...
    <div className="wrap reveal">
      ...
      <header className="masthead">
        ...
        <div className="masthead-meta">
          ...
          <span>FILL-ON-SUBMIT · NO T+1 · MIXED CURRENCY</span>
          <span>
